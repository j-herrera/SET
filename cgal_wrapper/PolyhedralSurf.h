...
public:
  const Vector_3& get_unit_normal() const {
    return normal;
  }
  Vector_3& get_unit_normal() {
    return normal;
  }
...
//read_write
  ...
  reference operator[] (key_type f) const {
    return f.get_unit_normal();
  }
};
...
template <class TPoly>
Facet_PM<TPoly> get_fpm(boost::vertex_attribute_t, TPoly& ) {
  return Facet_PM<TPoly>();
}
...
//typedef boost::read_write_property_map_tag category;
  ...
  reference operator[] (key_type h) const {
    return h.len;
  }
};
